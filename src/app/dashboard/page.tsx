...
export default async function DashboardPage({
  ...
  const [user, proposals, statsAgg] = await Promise.all([
    prisma.user.findUnique({
      ...
      select: {
        ...
        profile: { select: { onboardingDismissed: true } },
      },
    }),
  ...
  const hasCreatedProposal = stats.total > 0;
  const hasSentProposal = stats.sent > 0;
  const hasSignature = stats.accepted > 0;
  const allOnboardingDone = hasCreatedProposal && hasSentProposal && hasSignature;
